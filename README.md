...
- **미니맵**: 캔버스 우상단 축소판 — 클릭/드래그로 화면 점프.
- **우클릭 메뉴**: 부품(편집/회전/복제/잠금/삭제) · 빈곳(붙여넣기/전체선택/화면맞춤).
- **다중선택 일괄 편집**: 배선 여러 개 → 색상·SQ·두께·전원 일괄, 부품 여러 개 →
  타입·글자방향·잠금 일괄. 
- **도면 프레임**: 전장 설정 체크 → 2중 도곽(인쇄·PNG·DXF 포함).
...
## 계통도(단선도) · PLC I/O · 케이블표

- **계통도**: 팔레트 하단 **SYM(계통도 심볼)** — 차단기/누전/퓨즈/MC/THR/변압기/
  모터/램프/단로기/접지/CT/계측기/보조접점 a·b/코일/PB/3상/버스바(모선).
  ＋시트로 "계통도" 시트를 만들고 심볼을 세로로 배치 → ⚯ 배선으로 위/아래 단자
  결선. **DXF 내보내기 시 심볼이 실선 그대로** 나갑니다(SYM 레이어).
- **PLC I/O 리스트(`I/O`)**: PLC 부품의 단자별 연결 라인번호·상대 기기/단자/타입
  CSV. 단자 이름을 X0/Y0처럼 지으면 주소 컬럼이 됩니다.
- **케이블표(`케이블표`)**: 전장↔기계(센서·모터 등 필드) 배선만 모아 판넬측/현장측
  정리 + 길이 합계 CSV.
